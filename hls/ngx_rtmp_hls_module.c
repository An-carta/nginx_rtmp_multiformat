...
typedef struct {
    ...
    ngx_str_t                           key_url;
    ngx_uint_t                          frags_per_key;
    ngx_flag_t                          enable_multiformat;  // This will store the state of the enable_multiformat (on/off)
    ngx_array_t                         *formats;  // array of ngx_str_t
...
static ngx_command_t ngx_rtmp_hls_commands[] = {
    ...
    // added directive  
    { ngx_string("hls_multiformat"),      // Name of the directive
      NGX_RTMP_MAIN_CONF | NGX_RTMP_SRV_CONF | NGX_RTMP_APP_CONF | NGX_CONF_1MORE,    // Configuration flag: Takes one argument (on/off)
      ngx_rtmp_hls_multiformat,   // Handler function for the directive
      NGX_RTMP_APP_CONF_OFFSET,                                 // Global config
      0,                                 // No offset
      NULL },                            // Handler-specific data
...
//added handler funtion
static char *
ngx_rtmp_hls_multiformat(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
...
ngx_rtmp_hls_create_app_conf(ngx_conf_t *cf)
{
    ...
    // initialize my new fields
    conf->enable_multiformat = NGX_CONF_UNSET;      
    conf->formats = NULL;
